...
public class GameController {
    ...
    public ResponseEntity<Map<String, Object>> character(@AuthenticationPrincipal OAuth2User principal) {
        Map<String, Object> c = game.character(userId(principal));
    ...
    public ActionResult attack(@AuthenticationPrincipal OAuth2User principal, @RequestBody(required = false) AttackRequest req) {
        return game.attack(userId(principal), req == null ? null : req.target());
    }
    ...
    public ActionResult cast(@AuthenticationPrincipal OAuth2User principal, @RequestBody CastRequest req) {
        return game.cast(userId(principal), req.spell(), req.target());
    }
    ...
    public ActionResult prepare(@AuthenticationPrincipal OAuth2User principal, @RequestBody SpellRequest req) {
        return game.prepare(userId(principal), req.spell());
    }

    @PostMapping("/delve/quaff")
    public ActionResult quaff(@AuthenticationPrincipal OAuth2User principal) {
        return game.quaff(userId(principal));
    }
    ...
    public ActionResult hire(@AuthenticationPrincipal OAuth2User principal, @RequestBody HireRequest req) {
        return game.hire(userId(principal), req.cls(), req.name());
    }
    ...
    public ActionResult dismiss(@AuthenticationPrincipal OAuth2User principal, @RequestBody NameRequest req) {
        return game.dismiss(userId(principal), req.name());
    }
    ...
    public record AttackRequest(Integer target) {}

    public record CastRequest(String spell, Integer target) {}

    public record SpellRequest(String spell) {}

    public record HireRequest(String cls, String name) {}
